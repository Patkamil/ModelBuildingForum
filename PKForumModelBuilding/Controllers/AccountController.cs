    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //SendEmail(model.Email, "Powitanie na Forum Modelarskim", "Witamy i dziękujemy za zarejestrowanie się na naszym forum. Życzymy miłego pobytu.");
                    await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);

                    Profile profile = new Profile { UserName = model.Email, CreateData = DateTime.Now};
                    ForumModelarskieContext db = new ForumModelarskieContext();
